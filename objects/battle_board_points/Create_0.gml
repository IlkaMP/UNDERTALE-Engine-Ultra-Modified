...
// 用于绘制边框
function drawBorder() {
    var _surface = cover ? battle_board._surface_board_cover : battle_board._surface_board_extra;
    ...
    if (rect && battle_board.edge) {
        ...
        draw_sprite_ext(spr_battle_board_edge, global.classic_ui_spr, left[0], left[1], 1, 1, angle, battle_board.color_frame, battle_board.alpha_frame);
        draw_sprite_ext(spr_battle_board_edge, global.classic_ui_spr, right[0], right[1], -1, 1, angle, battle_board.color_frame, battle_board.alpha_frame);
        draw_sprite_ext(spr_battle_board_edge, global.classic_ui_spr, up[0], up[1], 1, -1, angle, battle_board.color_frame, battle_board.alpha_frame);
        draw_sprite_ext(spr_battle_board_edge, global.classic_ui_spr, down[0], down[1], -1, -1, angle, battle_board.color_frame, battle_board.alpha_frame);
        ...
        for (var i = 0; i < ds_list_size(listVertex); i++) {
            ...
            draw_sprite_ext(spr_pixel, 0, x + ax - (5 - 4 * battle_board.edge) * cos(degtorad(floor(dir))), y + ay + (5 - 4 * battle_board.edge) * sin(degtorad(floor(dir))), 5, point_distance(ax, ay, bx, by) + 5 + 5 * cos(degtorad(dir % 45)) - 2 * 4 * battle_board.edge, dir + 90, battle_board.color_frame, battle_board.alpha_frame);
        }
    } else {
        var size = ds_list_size(listDivideIndex);
